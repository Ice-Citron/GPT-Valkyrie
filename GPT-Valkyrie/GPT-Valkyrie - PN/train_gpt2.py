...
class SyncPowerFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, bias, running_phi, eps, afwd, abkw, ema_gz, warmup_iters, current_iter, process_group, affine):
        ...
        if affine:
            y = weight.view(1, 1, C) * y + bias.view(1, 1, C) # Shape: (B, T, C)        # CHANGE: Use view for weight and bias, maintain B, T, C shape // https://claude.ai/chat/0f44cae4-e15c-4c3d-acd1-7fef7701356b
